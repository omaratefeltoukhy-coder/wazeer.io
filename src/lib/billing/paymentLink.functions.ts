...
function validateReturnUrl(url: string): string {
  ...
  if (!siteUrl) return url; // Can't validate without knowing our domain
  const allowed = new URL(siteUrl).hostname;
  try {
  ...
  throw new Error("Invalid return_url: must be a subdomain of the site's domain.");
}
  ...
  .handler(async ({ data }) => {
    try {
      ...
      const body = {
        ...
        checkout: data.return_url ? { url: validateReturnUrl(data.return_url) } : { url: null },
